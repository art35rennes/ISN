...
def Tir_aleat():
    ...
    Coord_Tir_1 = random.choice(Grille)
    print("Tir aleat " + str(Coord_Tir_1))
    Grille.remove(Coord_Tir_1)
...
def Tir_IA():
    ...
    global Grille, Boat_found, Boat_direction
...
def Verif_joueur_toucher():
    ...
    global Grille, Boat_found, Boat_direction, Boats_joueur, Coord_Tir_2, Coord_Tir_1
    
    if [key for key in Boats_joueur if Coord_Tir_2 or Coord_Tir_1 in Boats_joueur[key]] != []:

        print("                                         **Bateau toucher"+ " " + coordIA)

        Toucher = Toucher + 1
        toucher = "non"
        bateautouche = [key for key in Boats_joueur if coordIA in Boats_joueur[key]]
...
def Verif_deja_tirer():
    ...
    global Coord_Tir_2, Coord_Tir_1, Grille
